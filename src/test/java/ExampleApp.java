...
import com.jquestrade_room.*;
import com.jquestrade_room.Balances.Currency;
import com.jquestrade_room.Candle.Interval;
...
class ExampleApp {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(q.getAuthorization().getAccessToken());
			
			System.out.println(q.getLastRequest());
			String acc = q.getAccounts()[0].getNumber();
			q.getActivities(acc, ZonedDateTime.now().minusWeeks(1), ZonedDateTime.now());
			
			q.getBalances(acc);
			
			
			
			q.getExecutions(acc, ZonedDateTime.now().minusWeeks(1), ZonedDateTime.now());
			
			q.getMarkets();
			
			q.getSymbol("BMO");
			
			Symbol[] a = q.searchSymbol("BMO");
			
			q.getCandles(a[0].getSymbolId(), ZonedDateTime.now().minusWeeks(1), ZonedDateTime.now(), Interval.OneDay);
			
			System.out.println(a[0].getCurrency());
			System.out.println(a[0].getDescription());
			System.out.println(a[0].getSymbol());
			System.out.println(a[0].getSecurityType());
			System.out.println(a[0].getListingExchange());
